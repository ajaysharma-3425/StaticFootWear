...
export default function KidsBrandCatalogPage() {
  const params = useParams();
  const brandSlug = (params?.brand as string) || "";

  const currentBrand = kidsBrands.find(b => b.id.toLowerCase() === brandSlug.toLowerCase());

  const brandProducts = currentBrand
    ? products.filter(p => currentBrand.productIds.includes(p.id) && p.category === 'kids')
    : [];
  ...
  return (
    <main className="min-h-screen bg-[#030303] text-white pb-20">
      <section className="container mx-auto px-4 md:px-10 pt-20 pb-10">
        <Link href="/kids" className="inline-flex items-center gap-2 text-[#d4af37] text-[10px] font-black uppercase tracking-[0.3em] mb-6 hover:opacity-70">
          <ArrowLeft size={16} /> Back to Kids' Section
        </Link>
