...
import { womenBrands } from '../../data/brand';  // flat array, not category object
import { ArrowRight } from 'lucide-react';

export default function WomenBrandPage() {
  return (
    <main className="min-h-screen bg-[#030303] text-white pt-32 pb-20">
      <div className="container mx-auto px-4">
        {/* Header Section */}
        <div className="mb-16 text-center">
          <span className="text-[#d4af37] text-xs font-black tracking-[0.5em] uppercase mb-4 block">Exclusive Catalog</span>
          <h1 className="text-5xl md:text-7xl font-black italic tracking-tighter uppercase">
            Women's <span className="text-transparent bg-clip-text bg-gradient-to-r from-[#d4af37] to-[#d4af37]/60">Brands</span>
          </h1>
        </div>

        {/* Direct grid of all brands (no category grouping) */}
        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6 gap-4">
          {womenBrands.map((brand) => (
            <Link href={`/women/brand/${brand.id}`} key={brand.id}>
              <motion.div
